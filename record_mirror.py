#!/usr/bin/env python3
"""
Bimanual MIRROR teleoperation data recording script.
...
Usage:
  python record_mirror.py
  python record_mirror.py --dataset my_dataset --task "Pick and place" --episodes 20
"""
...
class MirrorActionProcessor(RobotActionProcessorStep):
    ...
    def action(self, action: RobotAction) -> RobotAction:
        ...
        for key, value in action.items():
            ...
            # Negate mirror-axis joints
            joint_name = new_key.split("_", 1)[-1].removesuffix(".pos")
...
# ──────────────────────────────────────────────────────────────────────────────
DEFAULT_DATASET   = "local/bimanual_so101_mirror_demo"
...
def main():
    parser = argparse.ArgumentParser(description="Bimanual SO101 MIRROR data recorder")
    ...
    # ── Rerun visualizer ──────────────────────────────────────────────────────
    if display_data:
        init_rerun(session_name="recording_mirror")
